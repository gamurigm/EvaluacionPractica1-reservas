...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|r|}
        ...
        100               & 0.12 s                   & 0.0\%               & Estable \\ \hline
        250               & 12.6 s                   & 0.0\%               & Estable \\ \hline
        500               & 17.2 s                   & 0.6\%               & Límite \\ \hline
        1000              & 10.3 s                   & 24.0\%              & Degradado \\ \hline
    \end{tabular}
    \caption{Resultados comparativos de carga con JMeter}
    \label{tab:jmeter_multi_results}
\end{table}

El análisis de los resultados de JMeter (visualizados en las capturas de la Sección de Anexos) confirma la robustez del sistema bajo carga nominal. Con una carga de 100 usuarios, el sistema respondió de manera óptima con un 0.0\% de error. Al incrementar a 250 usuarios, la latencia subió a 12.6s pero se mantuvo estable sin fallos. A los 500 usuarios, el sistema empezó a mostrar signos de fatiga con ligeros errores (0.6\%). Finalmente, bajo carga de estrés de 1000 usuarios, el sistema experimentó una degradación significativa con un 24.0\% de errores, confirmando el punto de saturación de recursos.
...
\subsubsection{Evidencia de Integración Continua}
Tras superar retos técnicos iniciales relacionados con la conectividad de la base de datos en entornos virtualizados y la compatibilidad de versiones de las librerías de prueba, se logró la ejecución exitosa del pipeline. Como se observa en la Figura \ref{fig:github_actions_success}, todos los jobs (unit tests, functional tests y SonarQube scan) finalizaron correctamente.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/CICD.png}
    \caption{Ejecución exitosa del Pipeline de CI/CD en GitHub Actions}
    \label{fig:github_actions_success}
\end{figure}

Para alcanzar este estado, se implementaron las siguientes soluciones técnicas:
\begin{itemize}
    \item \textbf{Contenerización de Base de Datos:} Se añadió un servicio de MongoDB dentro del runner de Ubuntu, incluyendo un \textit{health check} mediante \texttt{mongosh} para asegurar que el servicio esté listo antes de iniciar las pruebas.
    \item \textbf{Estabilización de Dependencias:} Se ajustaron las versiones de \textbf{Jest} y \textbf{Supertest} a versiones estables (v29 y v6 respectivamente) para evitar errores de compatibilidad en el entorno de GitHub.
    \item \textbf{Gestión de Secretos:} Se configuraron las variables de entorno para que el análisis de SonarQube sea opcional (\texttt{continue-on-error}), permitiendo que el flujo principal continúe incluso si el servicio externo de análisis estático presenta latencia.
\end{itemize}
